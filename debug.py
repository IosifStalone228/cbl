...
function toggleLSOAs(wardCode) {
    fetch('/assets/lsoa_mapping.json')
        ...
        .then(data => {
            ...
            if (lsoaLayers[wardCode]) {
                window.map.removeLayer(lsoaLayers[wardCode]);
                delete lsoaLayers[wardCode];
            } else {
